...
function OutcomesTable() {
    ...
    const exportToCsv = () => {
        ...
        // Create a Map to group answers by section and maintain insertion order
        let groupedAnswers: Map<string, Answers[]> = new Map();

        // Group the answers by their section
        answers.forEach((answer) => {
            if (!groupedAnswers.has(answer.section)) {
                groupedAnswers.set(answer.section, []);
            }
            groupedAnswers.get(answer.section)?.push(answer);
        });

        // Sort the answers within each section by q_id
        groupedAnswers.forEach((answersArray) => {
            answersArray.sort((a, b) => a.q_id - b.q_id);
        });

        // Flatten the grouped answers back into a single array while maintaining the original section order
        groupedAnswers.forEach((answersArray, section) => {
            answersArray.forEach((answer) => {
                let header =
                    "Section " + answer.section + " Q_ID " + answer.q_id;
